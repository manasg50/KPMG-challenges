...
But for the URL which contain deeper layer of end-point, this function will identify it and 
traverse deeper into tree and construct the data structure.
...
def traverse_tree(url, data_path):
    ...
    for path in data_path:
        ...
        # For example we got 'http://169.254.169.254/latest/meta-data' in first call. 
        # When the recursive call is made to this function then the URL will be 'http://169.254.169.254/latest/meta-data and the path will be ami-id. Now the new_url would be 'http://169.254.169.254/latest/meta-data/ami-id
        # new_url = url + path
        ...
        if path.endswith("/"):
            ...
            # To create an array out of obtained values. 
            #'http://169.254.169.254/latest/meta-data/block-device-mappping/' will give ['ami','root'].
