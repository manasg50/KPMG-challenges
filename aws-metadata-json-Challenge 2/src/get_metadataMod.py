...
def fetch_key_value(metadata, search_key):
    key_value = None
            ...
            if isinstance(value, list):
                key_value_list = list(
                                      filter(
                                            lambda x: x != None,
                                            [
                                                fetch_key_value(item,search_key) for item in value
                                            ]
                                            )
                                     )
                key_value = key_value_list[0] if len(key_value_list) > 1 else None
            elif isinstance(value, dict):
                key_value = fetch_key_value(value, search_key)
                if key_value:
                    break
    return key_value
...
"""
def get_metadata_json(key=None):
    metadata = get_metadata()
    if key:
        metadata = fetch_key_value(metadata, key)
    metadata_json = json.dumps(metadata, indent=4, sort_keys=True)
    return metadata_json

if __name__ == '__main__':
     key = "block-device-mapping"
     key1 = "macs"
     key2 = "ipv4-associations"
     key3 = "65.2.140.213"
     key4 = "system"
     ...
     print(get_metadata_json(key3))
     print(get_metadata_json(key4))
